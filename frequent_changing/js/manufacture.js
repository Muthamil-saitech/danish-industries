$(document).ready(function () {
    ...
    $(document).on("click", ".print_route_card", function () {
        viewRouteCard($(this).attr("data-id"));
    });

    function viewRouteCard(id) {
        open(
            base_url + "print_route_card_detail/" + id,
            "Print Route Card",
            "width=1600,height=550"
        );
        newWindow.focus();
    ...
    $(document).on("click", ".print_job_card", function () {
        viewJobCard($(this).attr("data-id"));
    });

    function viewJobCard(id) {
        open(
            base_url + "print_job_card_detail/" + id,
            "Print Route Card",
            "width=1600,height=550"
        );
        newWindow.focus();
