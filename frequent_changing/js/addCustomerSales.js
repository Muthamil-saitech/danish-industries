$(document).ready(function () {
    ...
    $("#customer_quote_form").submit(function (e) {
        let status = true;
        let focus = 1;
        let quotation_no = $("#quotation_no").val();
        let customer_id = $("#customer_id").val();
        // let customDatepicker = $("#quote_date").val();
        // let statusField = $("#status").val();
        let product_id = $("#product_id").val();
        // let paid = $("#paid").val();
        // let accounts = $("#accounts").val();
        let date = $("#quote_date").val();

        if (date == "") {
            status = false;
            showErrorMessage("quote_date", "The Quotation Date field is required");
        } else {
            $("#quote_date").removeClass("is-invalid");
            $("#quote_date").closest("div").find(".text-danger").addClass("d-none");
        }

        if (quotation_no == "") {
            status = false;
            showErrorMessage("quotation_no", "The Quotation No field is required");
        } else {
            $("#quotation_no").removeClass("is-invalid");
            $("#quotation_no").closest("div").find(".text-danger").addClass("d-none");
        }
