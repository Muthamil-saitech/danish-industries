$(document).ready(function () {
    ...
    $('#stockCheck').on('shown.bs.modal', function () {
        ...
        if (Object.keys(rmQuantities).length === 0) {
            $(".stock_check_table").removeClass('d-none').html("<p class='text-center my-2 text-danger'>Please add material first</p>");
            $(".download_button").addClass("disabled");
            return;
        }
        let sn = 1;
            ...
            success: function (data) {
                if (data && data.length > 0) {
                    data.forEach(function(item) {
                        ...
                        neededForManufacture = neededForManufacture < 0 ? 0 : neededForManufacture;
                    ...
                    $(".stock_check_table").removeClass("d-none").html('<p class="text-center my-2 text-danger">No Stock Materials Available!</p>');
                }
            },
            error: function () {
                $(".stock_check_table").removeClass("d-none");
                generateTable(productName, customerName, productCode, currentStock, needed, unitName);
            }
        });
    });
    ...
    $(document).on('change', 'input[type=radio][name^="inter_state"]', function () {
    ...
    $(document).on('change', '#order_type', function () {
        ...
        $.ajax({
            ...
            success: function (data) {   
                console.log("data",data);
                let tax_rate = data || 0;
                let interStateVal = $('input[type=radio][name^="inter_state"]:checked').val() || 'N';
                let isYes = interStateVal === 'Y';
    ...
    $(document).on('change', '.pquantity_c, .sale_price_c, .cgst_input, .sgst_input, .igst_input', function () {
        let row = $(this).closest('tr');
        let hidden_base_url = $("#hidden_base_url").val();
        // let interStateVal = row.find('input[type=radio][name^="inter_state"]:checked').val() || 'N';
        // let isYes = interStateVal === 'Y';
    ...
    function update_invoice_totals() {
        ...
        $(".invoice_amount_c, .due_amount_c, .order_due_amount_c").val(total_sub_total.toFixed(2));
        modalIssueFix();
    }
    ...
    $(document).on("click", "#finishProduct", function (e) {
        ...
        $(".add_trm").append(
            "<tr>" +
                ...
                default_currency +
                '</span></div><span class="text-success" id="up_'+ i +'"></span></td>' +
                '<td><input type="hidden" class="form-control tax_type_id" name="tax_type[]" id="tax_type_id_' + i + '" value="' + tax_type_id + '" readonly placeholder="Tax Type"><span class="selected_taxtype">'+ selectedOrderType +'</span></td>' +
                '<td><div class="form-group radio_button_problem"><div class="radio">' +
                '<label><input type="radio" name="inter_state[' + i + ']" id="inter_state_yes_' + i + '" value="Y" ' + 
                    (i === 1 ? 'checked' : (firstInterState === 'Y' ? 'checked disabled' : 'disabled')) + '> Yes</label>&nbsp;' +
                '<label><input type="radio" name="inter_state[' + i + ']" id="inter_state_no_' + i + '" value="N" ' + 
                    (i === 1 ? 'checked' : (firstInterState === 'N' ? 'checked disabled' : 'disabled')) + '> No</label>' +
                ((i !== 1) ? '<input type="hidden" name="inter_state[' + i + ']" value="' + firstInterState + '">' : '') +
                '</div></div></td>'+
                '<td class="cgst_cell" style="display:none;"><input type="text" name="cgst[]" class="form-control cgst_input" id="cgst_' + i + '" readonly></td>' +
                '<td class="sgst_cell" style="display:none;"><input type="text" name="sgst[]" class="form-control sgst_input" id="sgst_' + i + '" readonly></td>' +
                '<td class="igst_cell" style="display:none;"><input type="text" name="igst[]" class="form-control igst_input" id="igst_' + i + '" readonly></td>' +
        ...
        $.ajax({
            ...
            success: function (data) {   
                console.log("data",data);
                let tax_rate = data || 0;
                let interStateVal = newRow.find('input[type=radio][name^="inter_state"]:checked').val() || 'N';
                let isYes = interStateVal === 'Y';
    ...
    */
     ...
     * @param {Array} needed
     */
    function generateTable(productName, customerName, productCode, currentStock, needed, unitName) {
        let sn = 1;
        let table = "";
        for (let i = 0; i < productName.length; i++) {
            ...
            table +=
                ...
                sn +
                "</td><td>" +
                productName[i] + " (" + productCode[i] + ")" +
                "<br><small class='text-muted'>" + custName + "</small>" +
                "</td><td>" +
                currentStock[i] + " "+unitName[i] +
                "</td><td>" +
                needed[i] + " " + unitName[i] +
                "</td></tr>";
            sn++;
    ...
    $(document).on("click", ".order_submit_button", function () {
        ...
        if (order_type == "") {
            showErrorMessage("order_type", "The Nature of Business field is required");
            status = false;
        } else {
            ...
            $("input[name='delivery_date_product[]']").each(function () {
                ...
                if (deliveryDate === "") {
                    ...
                    if (!$(this).next(".text-danger").length) {
                        $(this).after('<div class="text-danger">Delivery date is required</div>');
                    }
                    hasError = true;
                } else {
