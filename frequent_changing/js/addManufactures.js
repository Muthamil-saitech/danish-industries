$(document).ready(function () {
    ...
    $(document).on("click", "#pr_go", function (e) {
        ...
        if (!Number($(".product_quantity").val())) {
            ...
            $.ajax({
                ...
                data: { id: fproduct_id, value: product_quantity, stk_mat_type:stk_mat_type, selected_customer_id:selected_customer_id,customer_order_id:customer_order_id },
                dataType: "json",
                success: function (data) {
                    if(data.result==true) {
                        $(".submit_btn").removeClass("disabled");
    ...
    $(document).on("click", "#checkStockButton", function (e) {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (data) {                
                let table = '<table class="table table-bordered table-striped"><thead><tr><th>Raw Material</th><th>Stock</th><th>Required Quantity</th><th>Shortage</th></tr></thead><tbody>';
                if (data.length === 0) {
                    table += '<tr><td colspan="4" class="text-center text-danger">No Stock Material Available.</td></tr>';
                } else {
                    for (let i = 0; i < data.length; i++) {
                        ...
                        table +=
                            ...
                            data[i].name +
                            "</td><td>" +
                            data[i].stock_final + data[i].unit +
                            "</td><td>" +
                            data[i].required + data[i].unit +
                            "</td><td>" +
                            shortage + data[i].unit +
                            "</td></tr>";
    ...
    function checkSingleMaterialStock(material_id, quantity) {
        ...
        $.ajax({
            ...
            data: { material_id: material_id, quantity: quantity,stk_mat_type: stk_mat_type, fproduct_id: fproduct_id, customer_order_id: customer_order_id,selected_customer_id: selected_customer_id },
            dataType: "json",
            async: false,
            success: function (data) {
    ...
    $(document).on("submit", "#manufacture_form", function (e) {
        ...
        let stageCheckValue = $("input[name='stage_check']:checked").val() || $("input[name='stage_check_hidden[]']").val();        
        ...
        if (stk_mat_type == "") {
            ...
            showErrorMessage(
                "stk_mat_type",
                "The Material Type field is Required"
            );
        } else {
        ...
        if (statusField == "") {
            ...
            status = false;
            showErrorMessage("m_status", "You cannot save as draft if the start date is today.");
        } else {
        ...
        if (!Number(rowCount)) {
            ...
            showErrorMessage(
                "fproduct_id",
                "The Part Name field is Required"
            );
        } else {
        ...
        $.ajax({
            ...
            success: function (data) {   
                ...
                if (data.length === 0) {
                    ...
                    table += '<div class="d-flex justify-content-between"><h5 class="modal-title ms-2">'+data[0].stage_name+'</h5><div id="move_next_task_wrapper" class="form-check mb-1 '+ (data[0].status == 0 ? 'd-none' : '') +'"><input class="form-check-input move_next" type="checkbox" id="move_next_task"'+(data[0].move_to_next === "Y" ? "checked disabled" : "" )+'><label class="form-check-label" for="move_next_task">Move Next Task</label><p class="text-success d-none" id="check-msg">Checked!</p></div></div><hr><table class="table table-bordered table-striped"><thead><tr><th>Employee Name</th><th>Start Date</th><th>Complete date</th><th>Status</th><th>Note</th></tr></thead><tbody>';
                    for (let i = 0; i < data.length; i++) {
                        let statusesOptions = '';
                        ...
                        statusesOptions = `<option value="" ${pendingSelected}>Pending</option>` + statusesOptions;
