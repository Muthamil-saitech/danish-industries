...
$(document).on("click", "#customer_io", function (e) {
  ...
  let firstInterState = $(".add_cio").append(
    "<tr>" +
      ...
      i +
      '"><option value="">Please Select</option>\n' +
      '<option value="Micrometer Screw Gauge(INS001)">Micrometer Screw Gauge(INS001)</option>\n' +
      '<option value="Digital Multimeter(INS002)">Digital Multimeter(INS002)</option>\n' +
      "</select></td>" +
...
$(document).on("click", "#partner", function (e) {
  ...
  $(".add_partner").append(
    "<tr>" +
      ...
      '"><option value="">Please Select</option>\n' +
      '<option value="Micrometer Screw Gauge(INS001)">Micrometer Screw Gauge(INS001)</option>\n' +
      '<option value="Bore Gauges 123">Bore Gauges(INS002)</option>\n' +
      "</select></td>" +
      '<td><input type="number" name="qty[]" class="check_required form-control integerchk qty_c" placeholder="Quantity" id="quantity_' +
      j +
      '"></td>' +
