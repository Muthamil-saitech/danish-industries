...
@section('content')
    <section class="main-content-wrapper">
        ...
        <div class="box-wrapper">
            ...
            <div class="table-box">
                <!-- form start -->
                {!! Form::model(isset($customerOrder) && $customerOrder ? $customerOrder : '', [
                    'id' => 'manufacture_form',
                    'method' => isset($customerOrder) && $customerOrder ? 'PATCH' : 'POST',
                    'enctype' => 'multipart/form-data',
                    'route' => ['customer-orders.store', isset($customerOrder->id) && $customerOrder->id ? $customerOrder->id : ''],
                ]) !!}
                @csrf
                    ...
                    {{-- <div class="row">
                        <div class="col-sm-12 mb-2 col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('mat_type') is-invalid @enderror select2" name="mat_type" id="chk_mat_type">
                        ...
                        <div class="col-sm-12 mb-2 col-md-4 d-none" id="chk_mat_cust_div">
                            <div class="form-group">
                                <label>@lang('index.customer') </label>
                                <select class="form-control @error('mat_cat_id') is-invalid @enderror select2" name="customer_id" id="chk_customer_id">
                                    <option value="">@lang('index.select')</option>
                                    @foreach ($stock_customers as $value)
                                        <option {{ old('customer_id') == $value->id ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->name.'('.$value->customer_id.')' }}</option>
                                    @endforeach
                                </select>
                                <div class="text-danger d-none"></div>
                                @error('customer_id')
