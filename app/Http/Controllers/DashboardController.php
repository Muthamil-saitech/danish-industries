...
class DashboardController extends Controller
{
    ...
    {
        $total = [
            'product' => FinishedProduct::where('del_status','Live')->count(),
            'rm' => RawMaterial::where('del_status','Live')->count(),
            'supplier' => Supplier::where('del_status','Live')->count(),
            'customer' => Customer::where('del_status','Live')->count(),
        ];
    ...
    public function moneyFlow()
    {
        ...
        for($i = 0, $iMax = count($previous_six_month); $i < $iMax; $i++) {
            ...
            // dd($purchase);
            $supplierDuePayment[] = Supplier_payment::whereMonth('purchase_date', $month)->whereYear('purchase_date', $year)->where('del_status', 'Live')->sum('amount');
            ...
            $customerDueReceive[] = CustomerDueReceive::whereMonth('order_date', $month)->whereYear('order_date', $year)->where('del_status', 'Live')->sum('total_amount');
