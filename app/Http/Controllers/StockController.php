...
class StockController extends Controller
{
    ...
    public function rawMaterialStockCheck(Request $request)
    {
        ...
        $stock = [];
        foreach ($rm as $r) {
            $currentStock = MaterialStock::where('mat_id', $r->rawMaterials->id)
                ->where('mat_cat_id', $r->rawMaterials->category)
                ->where('del_status', 'Live')
                ->first();
            // dd($currentStock);
            $stock[] = [
                ...
                'required' => $quantity,
                'status' => $currentStock->current_stock >= $quantity ? 'in_stock' : 'need_purchase',
            ];
        }
    ...
    public function rawMaterialStockCheckByMaterial(Request $request)
    {
        $rm_id = $request->rm_id;
        $quantity = $request->quantity;
        $stk_mat_type = $request->stk_mat_type;
        ...
        if (!empty($rm_id)) {
            foreach ($rm_id as $key => $value) {
                ...
                if($stk_mat_type=="1") {
                    $ms = MaterialStock::where('mat_id', $rm->id)->where('mat_cat_id', $rm->category)->where('mat_type',$stk_mat_type)->where('customer_id',$selected_customer_id)->where('del_status', 'Live')->first();
                } else {
                    $ms = MaterialStock::where('mat_id', $rm->id)->where('mat_cat_id', $rm->category)->where('mat_type',$stk_mat_type)->where('del_status', 'Live')->first();
                }
                // dd($ms);
                $consumptionQuantity = $quantity[$key];
                $stock[] = [
                    'id' => $rm->id,
                    'name' => $rm->name,
                    'stock_final' => $ms->current_stock,
                    'stock' => $ms->current_stock,
                    'required' => $consumptionQuantity,
                    'status' => $ms->current_stock >= $consumptionQuantity ? 'in_stock' : 'need_purchase',
                    'unit' => getRMUnitById($ms->unit_id),
                ];
            }
        }
        return response()->json($stock);
