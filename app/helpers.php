...
function getCustomerDue($id)
{
    ...
    $customerPayment = CustomerDueReceive::where('customer_id', $id)->where('del_status', 'Live')->sum('total_amount');
...
function getDrawerNo($id)
{
    $row = \App\Drawer::where('id', $id)->first();
    return isset($row->drawer_no) && $row->drawer_no ? $row->drawer_no : '-';
}

function getInsertType($id)
{
    $row = \App\RawMaterial::where('id', $id)->first();
    if(isset($row->insert_type) && $row->insert_type==1) {
        return '(Consumable)';
    } else if(isset($row->insert_type) && $row->insert_type==2) {
...
function getPurchaseSaleUnitById($id)
{
    ...
    return isset($row->name) && $row->name ? $row->name : 'KG';
}

function getStockUnitById($id)
{
    $row = \App\MaterialStock::where('id', $id)->first();
    $unit_id = isset($row->unit_id) && $row->unit_id ? $row->unit_id : '0';
    return getRMUnitById($unit_id);
}
/**
 * Get Product Name
 */
...
{
    return isset($id) && $id=="1" ? "Material" : "Raw Material";
}
/**
 * Get Customer Name
 */
...
function getCustomerCodeById($id)
{
    $row = \App\Customer::where('id', $id)->first();
    return isset($row->customer_id) && $row->customer_id ? $row->customer_id : '';
}

function getStockCustomerNameById($id)
{
    $row = \App\Customer::where('id', $id)->first();
    return isset($row->name) && $row->name ? $row->name : 'Danish';
}
/**
...
function getTaxItems($tax_type)
{

    return TaxItems::where('tax_type',$tax_type)->first();
}
...
function getCustomerOpeningDue($customer_id, $date)
{
    ...
    $opening_due_receive = \App\CustomerDueReceive::select(DB::raw('SUM(total_amount) as amount'))
        ->where('customer_id', $customer_id)
        ->where('date', '<', $date)
        ->where('del_status', 'Live')
        ->first();
...
{
    $customer_due_rec = \App\CustomerDueReceive::select('reference_no', 'total_amount')
        ->where('customer_id', $customer_id)
        ->where('date', $date)
        ->where('del_status', 'Live')
        ->get();
...
function getTotalCredit($id)
{
    ...
    // $customerDueReceive = CustomerDueReceive::where('account_id', $id)->where('del_status', 'Live')->sum('total_amount');

    $total_credit = $totalOpeningBalance + $deposit;
    return $total_credit;
}
...
if (!function_exists('getAdjustData')) {
    function getAdjustData($totalStock, $rmId)
    {
        ...
        $rmLog = StockAdjustLog::where('mat_stock_id', $rmId)->first();
...
if (!function_exists('getStageName')) {
    ...
    {
        $manufacture = Manufacture::where('customer_order_id', $id)->orderBy('id', 'desc')->first();
        if ($manufacture) {
            return $manufacture->stage_name;
        } else {
            return '';
        }
    }
...
if (!function_exists('getStageNo')) {
    function getStageNo($orderId)
    {
        $manufacture = Manufacture::where('customer_order_id', $orderId)->orderBy('id', 'desc')->first();
        if (!$manufacture) {
            return '';
        }
        return 'Stage ' . ($manufacture->stage_counter);
    }
}

if (!function_exists('getAssignee')) {
    function getAssignee($id,$mid)
    {
        $manufacture = Manufacture::where('id', $mid)->where('customer_order_id', $id)->orderBy('id', 'desc')->first();
        if ($manufacture) {
            $production_stage = ProductionStage::where('name',$manufacture->stage_name)->where('del_status','Live')->first();
            $scheduling = ProductionScheduling::where('manufacture_id',$manufacture->id)->where('production_stage_id',$production_stage->id)->where('del_status','Live')->first();
            // return $scheduling;
            if($scheduling) {
                $employee = User::where('id',$scheduling->user_id)->first();
                return $employee ? $employee->name.'('.$employee->emp_code.')' : '';
            } else {
                return '';
            }
        } else {
...
if (!function_exists('getEmpCode')) {
    function getEmpCode($user_id)
    {
        $employee = User::where('id',$user_id)->first();
        return $employee ? $employee->name . '(' . $employee->emp_code . ')' : 'N/A';
    }
}

if (!function_exists('substr_text')) {
    function substr_text($title, $str_length)
    {
...
function getheatNo($mat_id)
{
    $row = \App\MaterialStock::where('del_status', 'Live')->where('mat_id', $mat_id)->first();
    return $row->heat_no ?? '';
}
...
function getOrderPrice($price,$unit_price,$tax_type) {
    if($tax_type == 1 || $tax_type == '1') {
        return number_format($price+$unit_price, 2, '.', '');
    } else {
...
{
    return CustomerOrderDetails::where('del_status', 'Live')->where('customer_order_id', $order_id)->first();
}
function getSalePrice($price,$qty) {
    return number_format($price * $qty, 2, '.', '');
}

function getYourDCNo($manufacture_id) {
    $mrmaterial = \App\Mrmitem::where('manufacture_id', $manufacture_id)->where('del_status','Live')->first();
